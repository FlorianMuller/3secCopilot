...
## 0. Where we are (updated 2026-07-07)

- **Phase 1 (spike): DONE, passed on device** — see §10.1 for measured results. The spike lives in `modules/expo-montage/` (`spikeConcat`) with a dev-only harness at Settings → Advanced → "Export spike (dev)" (`src/features/Options/sections/ExportSpikeSection.tsx`).
- **Next: Phase 2 (UI shell)** — replace the Preview tab placeholder with the Export navigator: `PeriodList` + `ExportScreen` with stats & options (§7.1–7.3 / §9.1–9.3). Pure JS, no native work, ships without the module.
- After that, Phase 3 grows `spikeConcat` into the real `exportMontage` API (§7): day timeline with missing-day beats + opening card, chunking, progress events, cancellation.
