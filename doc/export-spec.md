...
- **Phase 5: DONE (sim-verified 2026-07-07, commits `dc061fa` native + `e235442` JS). All v1 phases complete — remaining work is the on-device checklist + post-v1 items.** `analyzeClips` is real (native `ClipAnalyzer`, metadata-only, network **disabled** — offloaded assets skipped and simply don't vote; bounded concurrency 6). JS: `computeQualityCombos` buckets by long edge (≤1280/≤1920/else → 720p/1080p/4K) × fps (≥45 → 60 else 30), default = mode with ties → lighter; quality picker + live size estimate on ExportScreen (loading state until analyze resolves); `startMontageExport` takes a `quality` combo (fixed-1080p30 removed). Preview (§9.4): 640×360 (portrait 360×640), fps capped 30, 1.5 Mbps ceiling, same overlay pipeline with proportional font px — layout matches the full render within rounding (§11.4 answered); output `cacheDirectory/exports-preview/` (replaced each render), inline expo-video player, invalidated on any output-affecting option change; native `mode:"preview"` accepted but a deliberate no-op (shortcuts arrive as ordinary settings). Preview and export share the one-export-at-a-time slot (mutually blocked in UI). New dev hooks: `EXPO_PUBLIC_AUTO_PREVIEW=1` (`[autopreview]` logs), `EXPO_PUBLIC_AUTO_QUALITY=<combo>`. Sim `verify-iphone` now permanently has a 720p60 testsrc video (via `simctl addmedia`) so the picker shows ≥3 combos. **Audio normalization (§11.3): DEFERRED post-v1** — the "balloons" case: honest per-clip gain needs a real PCM decode per trimmed range, AVAudioMix ramps must be fenced around click segments/empty ranges, and the beats-only silence path bypasses the mix. Verified on sim: analyze/combos/default correct vs ffprobe ground truth, estimate matches the §4.2 formula (264 MB for 130 s @1080p30), preview `DONE durationMs=48500` exact (card+17 clips+8 beats; 17 filled <20 → whole period, leading beats excluded — strict "between" reading of §9.4), 720p60 full-export regression exact (130000 ms, 1280×720@60), peakMB preview ~390 / 720p60 export ~540. Known VFR quirk (pre-existing, not a regression): 25 fps sources emit at source cadence through the 30 fps grid; durations/pts stay exact. Inline-player visual check is below the fold → device item.
