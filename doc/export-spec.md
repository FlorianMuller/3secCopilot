...
- **Phase 3 verification status**: end-to-end export **passes on the iOS simulator** — seeded 2026 period (189 items → 7 chunks, incl. beats-only chunks), monotonic progress stream, `DONE durationMs=134500` exactly matching the predicted timeline, output in `Documents/exports/`. Two native bugs were found by sim verification and fixed:
  ...
  Verification still pending (first task for the next session): ffprobe/frame/audio forensics of the output (card black, clips correct, no clicks at chunk boundaries), interactive cancel test, second-export no-overwrite, peakMB on device (sim dev-build reads ~875 MB incl. Metro; device release spike was ~325 MB).
- **Next: Phase 4 (beats & overlays)** — date/hour/title text layers (clips + missing-day beats), opening-card text, missing-day click sound (bundled resource). Then Phase 5 (preview mode, analyzeClips quality picker, polish). `doc/export-device-checklist.md` tracks what only a real device can prove.
